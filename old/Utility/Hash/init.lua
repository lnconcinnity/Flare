...
local function md5_feed_64(H, str, offs, size)
	-- offs >= 0, size >= 0, size is multiple of 64
	local W, K, md5_next_shift1 = common_W, md5_K, md5_next_shift
		...
		repeat
			...
			if d * d > p then
				...
				if idx < 16 then
					...
					R = mul(table.create(1, math.floor(R)), one, 1, 2)
					_, delta = mul(R, R, -1, 2)
					local hi1 = R[2] % 65536 * 65536 + math.floor(R[1] / 256)
					local lo1 = R[1] % 256 * 16777216 + math.floor(delta * TWO_POW_NEG_17 / root)
					local idx1 = idx % 8 + 1
					sha2_H_ext256[224][idx1] = lo1
					sqrt_hi[idx1], sqrt_lo[idx1] = hi1, lo1 + hi1 * hi_factor
					if idx1 > 7 then
			...
			if tail then
				...
				local function get_next_part_of_digest(bytes_needed)
					...
					if bytes_needed <= last_part_size then
						...
						local result1 = string.sub(last_part, 1, part_size_in_nibbles)
